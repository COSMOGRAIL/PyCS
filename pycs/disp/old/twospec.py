...
We define functions to calculate and handle 2D dispersion spectra, i.e. of 2 curves.

You can make multi-D plots elsewhere
...
import pycs.disp.twoopt


def calcd2s(lc1, lc2, timeshifts, dispersionmethod, fastret=True, optml=False):
	"""
	...
	optml = False : microlensing is not optimized for each step.
	Otherwise it is -- in this case lc1 and lc2 are changed !
	Note that applying microlensing before calling this will speed it up.
	
	We shift lc2, and we pass lc2 as second argument to the dispersionfunction.
	
	We return an array of the dispersion values.
	If fastret is true, we return only d2values; else there is also other stuff.
	"""

	lc2abstimeshifts = timeshifts + lc2.timeshift

	def d2(lc2abstimeshift):
		
		mylc1 = lc1.copy()
		mylc2 = lc2.copy()
		mylc2.timeshift = lc2abstimeshift # The choice of shifting lc2 or lc1 makes no fundamental difference at all
		
		if optml:
			pycs.disp.twoopt.opt_ml(mylc1, mylc2, dispersionmethod)
			
		if fastret:
			return dispersionmethod(mylc1, mylc2)["d2"]
		else:
			res = dispersionmethod(mylc1, mylc2)
	...
	if fastret:
		retstuff = vecd2(lc2abstimeshifts)
				
	else:
		bla = vecd2(lc2abstimeshifts)
...
def plot(lc1, lc2, dispersionmethod, timewidth=30, timestep=1.0, optml=False):
	"""
	...
	spectrum = calcd2s(lc1, lc2, timeshifts, dispersionmethod, fastret=False, optml=optml)
	...
	ax1.plot(timedelays, spectrum["d2"], "r.")
	...
	for tl in ax2.get_yticklabels():
    		tl.set_color('b')
	ax2.set_ylabel("Number of terms", size=16, color="b")	# this causes a crash, probably a bug...
...
# 		lc1c.bootstrap()
# 
# 		d2row = calcd2s(lc1c, lc2c, timeshifts, dispersionmethod, fastret=True, optml=optml)
